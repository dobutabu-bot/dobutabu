...
export async function POST(request: Request) {
  try {
    ...
    const preview = await buildCapitalImportPreview(user.id, fileFromForm(formData), {
    ...
    return Response.json({ message: "Mali bilgi önizlemesi oluşturulamadı. Dosya ve eşleme bilgilerini kontrol edin." }, { status: 500 });
  }
}
