...
const screenshotDir = path.join(process.cwd(), "artifacts", "final-action-runtime");

test.afterAll(async () => {
  await prisma.$disconnect();
});

test.describe("V4 final action runtime - gerçek UI aksiyonları", () => {
  test("edit, delete/archive, cancel, restore and reconciliation actions work through the UI", async ({ page }, testInfo) => {
...
test.setTimeout(600_000);
    ...
    const stamp = `FINAL-RUNTIME-TEST-${Date.now()}`;
    ...
    try {
      ...
      const advanceDescription = `${stamp} Masraf Avansı`;
      const updatedAdvanceDescription = `${advanceDescription} CRUD TEST GÜNCELLENDİ`;
      await gotoReady(page, "/advances?create=1");
      dialog = page.getByRole("dialog", { name: "Avans Hareketi Ekle" });
      await expect(dialog).toBeVisible();
      await openAdvanced(dialog);
      await dialog.getByLabel("Müvekkil", { exact: true }).selectOption(state.clientId);
      await dialog.getByLabel("Dosya", { exact: true }).selectOption(state.caseFileId);
      await dialog.getByLabel("Yön").selectOption("RECEIVED");
      await dialog.getByLabel("Tutar").fill("850.00");
      await dialog.getByLabel("Açıklama").fill(advanceDescription);
      await submitDialog(dialog, "Avans hareketi kaydet");
      await expect(page.getByText(advanceDescription).filter({ visible: true }).first()).toBeVisible();
      ...
      await expect(advanceMenu).toBeVisible();
      await expect(advanceMenu.getByRole("link", { name: "Görüntüle" })).toBeVisible();
      await expect(advanceMenu.getByRole("link", { name: "Belge bağla" })).toBeVisible();
      await advanceMenu.getByRole("button", { name: "Düzenle" }).click();
      dialog = page.getByRole("dialog", { name: "Avans Girişi Düzenle" });
      await expect(dialog).toBeVisible();
      await dialog.getByLabel("Tutar").fill("925.00");
      await dialog.getByLabel("Açıklama").fill(updatedAdvanceDescription);
      await submitDialog(dialog);
      await expect.poll(() => incomeAmount(state.advanceIncomeId)).toBe("925");
      await expect.poll(() => prisma.cashLedgerEntry.count({ where: { incomeId: state.advanceIncomeId } })).toBe(1);
      ...
      dialog = page.getByRole("dialog", { name: "Avans hareketi silinsin mi?" });
      await expect(dialog).toBeVisible();
      await dialog.getByRole("button", { name: "Sil", exact: true }).click();
      await expect(dialog).toBeHidden({ timeout: 30_000 });
      await expect.poll(() => incomeAndLedgerDeleted(state.advanceIncomeId)).toBe(true);
      await restoreFromDeletedRecords(page, "incomes", updatedClientName);
      await expect.poll(() => incomeAndLedgerDeleted(state.advanceIncomeId)).toBe(false);
      await screenshot(page, "04-advances-menu-edit-delete-restore.png");
      ...
      let suggestion = page.locator("article").filter({ hasText: `${stamp} Mutabakat Gideri` }).first();
      await suggestion.getByRole("button", { name: "Reddet" }).click();
      ...
      await reloadReady(page);
      suggestion = page.locator("article").filter({ hasText: `${stamp} Mutabakat Gideri` }).first();
      await suggestion.getByRole("button", { name: "Onayla" }).click();
      await expect.poll(() => bankRowMatchType(state.bankRowId)).toBe("AUTO_MATCHED");
      await reloadReady(page);
      await clickRowButton(page, `${stamp} Mutabakat Gideri`, "Bağlantıyı kaldır");
      await expect.poll(() => bankRowMatchType(state.bankRowId)).toBe("NONE");
      ...
      await testInfo.attach("runtime-network-errors", {
        body: Buffer.from(networkErrors.join("\n") || "none", "utf8"),
        contentType: "text/plain"
      });
      await testInfo.attach("runtime-console-errors", {
...
async function loginThroughUi(page: Page) {
  ...
  await page.getByLabel("E-posta", { exact: true }).fill(adminEmail);
...
async function createBankRowFixture(state: RuntimeState) {
  ...
  await prisma.cashLedgerEntry.create({
    data: {
      ...
      description: `${state.stamp} Manuel Mutabakat Hedefi`
    }
  });
...
async function cleanupRuntimeRecords(stamp: string) {
  ...
  await prisma.bankStatementRow.updateMany({
    where: { OR: [{ description: { contains: stamp } }, { importId: { in: bankImportIds } }] },
    data: { deletedAt }
  });
  await prisma.bankStatementImport.updateMany({ where: { id: { in: bankImportIds } }, data: { deletedAt } });
  await prisma.documentProcessingLog.updateMany({ where: { documentId: { in: documentIds } }, data: { deletedAt } });
  await prisma.document.updateMany({ where: { id: { in: documentIds } }, data: { deletedAt } });
  ...
  await prisma.assetValuation.updateMany({ where: { assetAccountId: { in: assetIds } }, data: { deletedAt } });
  await prisma.assetTransaction.updateMany({ where: { assetAccountId: { in: assetIds } }, data: { deletedAt } });
  await prisma.assetAccount.updateMany({ where: { id: { in: assetIds } }, data: { deletedAt, isActive: false } });
  await prisma.taskReminder.updateMany({ where: { id: { in: reminderIds } }, data: { deletedAt } });
  await prisma.invoiceOrReceipt.updateMany({ where: { id: { in: receiptIds } }, data: { deletedAt } });
  await prisma.income.updateMany({ where: { id: { in: incomeIds } }, data: { deletedAt } });
  await prisma.expense.updateMany({ where: { id: { in: expenseIds } }, data: { deletedAt } });
  await prisma.caseFile.updateMany({ where: { id: { in: caseIds } }, data: { deletedAt, archivedAt: deletedAt, status: "ARCHIVED" } });
  await prisma.client.updateMany({ where: { id: { in: clientIds } }, data: { deletedAt, archivedAt: deletedAt } });
  await prisma.cashAccount.updateMany({ where: { id: { in: cashAccountIds } }, data: { deletedAt, isActive: false } });
}
